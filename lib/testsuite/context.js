...
class Context extends EventEmitter {
  ...
  constructor({modulePath, settings, argv = {}, attributes = {}, virtuals = []}) {
    super();
  ...
  async loadModule() {
    ...
    const descriptionForCurrentModule = this.virtuals.find(
      ({filter}) => filter.test(this.modulePath)
    );

    // We have not to try to compile explicit ES module files into CommonJS
    // until we will be able to run it.
    this.__module = descriptionForCurrentModule
      ? await Utils.requireVirtualTest(this.modulePath, descriptionForCurrentModule)
