...
class Context extends EventEmitter {
  ...
  loadPostprocess() {
    const processors = this
      .settings
      .postprocess
      .map((relativeToCwdPath) => require(path.resolve(relativeToCwdPath)))
      .map((exports) => Array.isArray(exports) ? exports : exports.postprocess);

    return processors.flat();
  }

  async loadModule() {
    this.emit('pre-require', global);

    // We have not to try to compile explicit ES module files into CommonJS
    // until we will be able to run it.
    this.__module = this.modulePath.endsWith('.mjs') ||
      this.modulePath.endsWith('.mts') ||
      require(path.resolve('package.json')).type === 'module'
      ? await Utils.requireModule(this.modulePath)
      : await Utils.requireCompiledModule(this.modulePath, {
        postprocess: this.loadPostprocess()
      });
