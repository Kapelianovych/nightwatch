...
/**
 * A function that receives the output of the bundled file
 * and returns the code which will be executed.
 *
 ...
 * @property {Array<TransformFunction>} [postprocess] - an array of code
 *  transformers.
 */
 ...
 * @param {TransformOptions} [options] - options for tweaking the code
 *  transforming stage.
 * @returns {Promise<T>} the module exported values.
 */
const requireCompiledModule = async function (
  modulePath,
  {
    postprocess = []
  } = {}
  ...
  const code = await postprocess.reduce(
    (accumulator, process) => accumulator.then(process),
