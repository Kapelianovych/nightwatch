  ...
  static load(pluginName) {
    try {
      ...
      if (isFolder) {
        ...
        try {
          const descriptions = Utils.getDefaultExport(require(
            path.join(dirName, 'nightwatch', 'virtuals.js')
          ));

          plugin.virtuals = Array.isArray(descriptions)
            ? descriptions
            : [descriptions];
        } catch (error) {
          plugin.virtuals = false;
        }
      }
